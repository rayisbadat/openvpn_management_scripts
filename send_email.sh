...
send_welcome_letter_png() {
    ...
    cat $VPN_BIN_ROOT/templates/creds_template.txt | envsubst | mutt $vpn_email -e "set realname='$EMAIL'"  -s "$CLOUD VPN Configuration Files: $CLOUD_NAME" -a/tmp/$vpn_username.zip $VPN_FILE_ATTACHMENTS
}
